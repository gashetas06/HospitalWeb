...
def dashboard_paciente(request):
    ...
    proxima_cita = citas_qs.filter(fecha__gte=hoy, estado='Programada').first()
    ...
    try:
        internacion_activa = Internacion.objects.select_related(
            'idcama', 'idcama__iddepartamento'
        ).get(idpaciente_id=paciente.idpaciente, fechaegreso__isnull=True)
    ...
    return render(request, 'pacientes/dashboard_paciente.html', context)
...
def mis_citas_paciente(request):
    ...
    return render(request, 'pacientes/mis_citas_paciente.html', context)
...
def mi_historia(request):
    ...
    return render(request, 'pacientes/mi_historia.html', context)
...
def cambiar_password_paciente(request):
    ...
    return render(request, 'pacientes/cambiar_password_paciente.html', {
        'paciente': _get_paciente(request)
    })